...
\begin{abstract}
\noindent
Steady-state visually evoked potential (SSVEP)-based brain--computer interfaces (BCIs) translate a user's gaze at a flickering visual target into a discrete command, without requiring muscular activity. Over the last two decades, the decoding methods used to recognize the target frequency have evolved from correlation-based statistical techniques to deep neural networks, and the reported classification accuracy and information transfer rate (ITR) have both improved substantially. This report first reviews this literature (Sections~1--3), introducing the SSVEP-BCI problem, organizing the reviewed decoding methods along the standard BCI processing pipeline and Zerafa \textit{et al.}'s training-requirement taxonomy, surveying the state of the art within each resulting category, and arguing that ``robustness'' in this field spans three distinct dimensions: within-trial noise robustness, cross-subject and low-calibration robustness, and adversarial robustness. It then proposes and motivates three concrete, testable robustness mechanisms (H1: dual-modality color-plus-frequency redundancy on the safety-critical STOP key; H2: trial-by-trial online adaptation; H3: an alternative code-modulated visual evoked potential, c-VEP, encoding) that target these dimensions using a physical five-key wheelchair control panel built for this project (Sections~4--5). Finally, it describes the panel's hardware assembly (Section~6) and a first single-subject prototype data-collection and analysis pass (Section~7), reports honestly on what has and has not yet been accomplished (Section~8), and concludes (Section~9). The single-subject data set discussed here is explicitly a feasibility prototype, not a final validation of H1--H3; that validation is identified as the project's next concrete step.
\end{abstract}
...
\midrule
Signal acquisition & \multicolumn{3}{p{10.8cm}}{-- (electrode montage, sampling rate, and stimulus hardware are chosen independently of the decoder's calibration requirement)} \\
\addlinespace
Preprocessing & \multicolumn{3}{p{10.8cm}}{-- (band-pass / notch filtering and artifact rejection are applied uniformly, before any subject-specific or pooled data enters the model)} \\
\addlinespace
...
TRCA / eTRCA~\cite{nakanishi2018trca}, eTRCA\allowbreak+\allowbreak sbCNN~\cite{wei2025etrcasbcnn}, t-SSVEPformer\allowbreak+\allowbreak JFPTS~\cite{ding2026jfpts}, TDCA~\cite{liu2021tdca}, EEGNet (per-subject fine-tuning)~\cite{lawhern2018eegnet} &
...
\addlinespace
Control interface/application & \multicolumn{3}{p{10.8cm}}{-- (a speller, wheelchair-control, or drone-control application layer can, in principle, sit on top of any decoder in the two columns to the left)} \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\begin{tikzpicture}[
  node distance=1cm and 0.15cm,
  every node/.style={font=\scriptsize},
  ev/.style={draw, rounded corners=1.5pt, align=center, minimum width=1.7cm, minimum height=0.85cm, inner sep=2pt}
]
  \draw[thick, -{Stealth[length=2.5mm]}] (-0.3,0) -- (13.6,0) node[right, font=\small] {year};

  \foreach \x/\yr in {0/2007, 1.8/2014, 3.9/2015, 5.8/2017, 7.6/2018, 9.4/2024, 11.4/2026}
    \draw (\x,0.05) -- (\x,-0.05) node[below, font=\scriptsize] {\yr};

  \node[ev, fill=blue!12, above=of {(0,0)}]  (cca) {CCA \\ \cite{lin2007cca}};
  \node[ev, fill=blue!12, above=of {(2.0,0)}] (msi) {MSI \\ \cite{zhang2014msi}};
  \node[ev, fill=blue!12, above=of {(3.9,0)}] (fbcca) {FBCCA \\ \cite{chen2015fbcca}};
  \node[ev, fill=green!15, above=of {(5.8,0)}, yshift=1.2cm] (trca) {TRCA \\ \cite{nakanishi2018trca}};
  \node[ev, fill=blue!12, above=of {(5.8,0)}] (jfpm) {JFPM \\ \cite{chen2015pnas}};
  \node[ev, fill=green!15, above=of {(7.6,0)}] (eegnet) {EEGNet \\ \cite{lawhern2018eegnet}};
  \node[ev, fill=orange!20, above=of {(9.4,0)}, yshift=1.2cm] (sutl) {SUTL \\ \cite{li2024sutl}};
  \node[ev, fill=green!15, above=of {(9.4,0)}] (hyb) {eTRCA+sbCNN \\ \cite{wei2025etrcasbcnn}};
  \node[ev, fill=green!15, above=of {(11.4,0)}] (jfpts) {JFPTS \\ \cite{ding2026jfpts}};

  \foreach \a/\b in {cca/msi, msi/fbcca, fbcca/jfpm, jfpm/trca, trca/eegnet, eegnet/hyb, hyb/jfpts}
    \draw[-{Stealth[length=2mm]}, gray] (\a) -- (\b);

  \node[below=1.9cm of {(6.5,0)}, align=left, font=\scriptsize] (legend) {
...
\textbf{Deep-learning-for-mobile-BCI review.} Zhang and Tao's 2026 narrative review synthesizes post-2023 deep-learning SSVEP decoding work around three engineering-oriented themes: lightweight model architectures (for example, a calibration-free model compressed to 0.23~MB, and a transfer-learning method whose trainable weights are only 0.013\% of a non-transfer model~\cite{zhang2026mobilereview}), ultra-short time-window decoding, and cross-subject transfer and low-calibration strategies~\cite{zhang2026mobilereview}. The authors group cross-subject approaches into source-target knowledge mmapping/instancetransfer, domain-invariant representation learning with unsupervised distribution alignment (the same general family as SUTL), and physiologically inspired data augmentation~\cite{zhang2026mobilereview}. They explicitly flag, as a limitation of the field rather than of any single paper, that studies rarely report complete cross-subject generalization curves under a shared evaluation protocol, which makes rigorous quantitative comparison across cross-subject methods difficult~\cite{zhang2026mobilereview}.
...
\begin{tabular}{@{}p{2.3cm} p{1.7cm} p{1.9cm} p{1.1cm} p{1.6cm} p{1.7cm} p{0.6cm}@{}}
\toprule
...
\small
\begin{tabular}{@{}p{2.4cm} p{3.6cm} p{2.9cm} p{2.6cm}@{}}
\toprule
...
\small
\begin{tabular}{@{}p{5.8cm} p{2.5cm} p{4.3cm}@{}}
\toprule
...
\begin{figure}[htbp]
\centering
\fbox{\parbox[c][6cm][c]{0.8\textwidth}{\centering \todofill{Circuit schematic showing the STM32F103C8T6 GPIO/timer outputs, the ULN2003AG driver channels, the LED clusters with current-limiting resistors, the photoresistor verification circuit, and the power supply chain (XL6009E1 $\rightarrow$ L7805CV). To be inserted once available.}}}
...
All code, firmware, and supporting materials for this project are maintained at: \todofill{GitHub repository URL to be added once the repository is created.} \url{https://github.com/TODO-USERNAME/TODO-REPOSITORY}
...
This project addresses robust command extraction from SSVEP signals for brain--computer interfaces, with an application to wheelchair control. A literature review of approximately 20 papers established that ``robustness'' spans three distinct dimensions -- within-trial noise, cross-subject/low-calibration, and adversarial robustness -- at very different levels of maturity in the existing literature. Building on this analysis, three concrete mechanisms were proposed: color-plus-frequency redundancy on a safety-critical command (H1), trial-by-trial online adaptation (H2), and a broadband, code-modulated alternative encoding (H3). A five-key stimulus panel was designed and built around an STM32 microcontroller, supporting both frequency-coded and pseudorandom-code-modulated stimulation, and a first single-subject prototype recording and analysis pass validated the SSVEP decoding pipeline while identifying and diagnosing a specific, literature-consistent obstacle to c-VEP decoding. The proposed mechanisms have not yet been empirically tested under controlled conditions; synchronized, multi-condition data collection is identified as the immediate next step.
